...
/**
 * Qwen (Alibaba DashScope) provider implementation (OpenAI-compatible)
...
class QwenProvider extends BaseLLMProvider {
  ...
  getAvailableModels() {
    return [
      ...
      { id: 'qwen-plus', name: 'Qwen Plus', description: 'Balanced quality and cost' },
      { id: 'qwen-turbo', name: 'Qwen Turbo', description: 'Fast and cost-effective' },
      { id: 'qwen-long', name: 'Qwen Long', description: 'Long-context variant (availability depends on account/region)' },
    ];
  }
  ...
  async chat(messages, options = {}) {
    ...
    try {
      logger.debug(`Qwen request: model=${model}, messages=${formattedMessages.length}, tools=${options.tools?.length || 0}`);
      const response = await this.withRetry(async () => {
        return await this.client.chat.completions.create(requestParams);
      });
      return this.parseResponse(response);
    } catch (error) {
      logger.error('Qwen chat error:', error);
    ...
    try {
      ...
      for (const tc of Object.values(toolCallDeltas)) {
        try {
          ...
          logger.warn('Failed to parse Qwen tool call arguments');
        }
      }
